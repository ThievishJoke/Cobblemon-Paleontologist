...
import net.hana.cobblemon_paleontologist.mixin.BrushItemAccessorMixin;
...
public class BetterBrushItem extends BrushItem
{
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks)
    {
        ...
        {
            HitResult hitResult = ((BrushItemAccessorMixin)this).callGetHitResult(playerEntity);
            if (hitResult instanceof BlockHitResult blockHitResult)
            {
                if (hitResult.getType() == HitResult.Type.BLOCK)
                {
                    ...
                    if (bl)
                    {
                        ...
                        ((BrushItemAccessorMixin)this).callAddDustParticles(world, blockHitResult, blockState, user.getRotationVec(0.0F), arm);
